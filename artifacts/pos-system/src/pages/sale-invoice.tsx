...
export default function SaleInvoice() {
  ...
  const { data: sale, isLoading, isError } = useGetSale(saleId, {
  ...
  const handleWhatsApp = () => {
    ...
    const msg = `Dear ${customerName}, please find your tax invoice ${sale.billNo} for ₹${total.toFixed(2)} from ${COMPANY.name}.\n\nView: ${url}`;
        ...
        <div className="bg-white border-2 border-black text-[12px] text-black" style={{ fontFamily: "Consolas, 'Courier New', monospace" }}>
          ...
          <div className="grid grid-cols-2 border-b-2 border-black">
            ...
            <div className="grid grid-cols-2">
              <div className="border-b border-r border-black p-1.5">
                ...
                <p className="font-semibold">{sale.billNo}</p>
              </div>
              <div className="border-b border-black p-1.5">
                <p className="text-[10px] text-gray-600">Dated</p>
                <p className="font-semibold">{new Date(sale.saleDate).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}</p>
              </div>
              <div className="border-b border-r border-black p-1.5">
                <p className="text-[10px] text-gray-600">Mode/Terms of Payment</p>
                <p>{sale.payments?.map((p: any) => p.paymentType).join(", ") || "Cash"}</p>
              </div>
              <div className="border-b border-black p-1.5">
                <p className="text-[10px] text-gray-600">Reference</p>
                <p>POS Counter Sale</p>
              </div>
          ...
          <div className="grid grid-cols-2 border-b-2 border-black">
            <div className="p-3 border-r-2 border-black">
              ...
              <p className="font-bold mt-0.5">{customerName}</p>
            ...
            <tbody>
              {items.map((d, i) => (
                <tr key={i} className="align-top">
                  ...
                  <td className="border-r border-black p-1.5">{d.name}</td>
              ...
              <tr className="border-t-2 border-black font-bold">
                ...
                <td className="border-r border-black"></td>
                <td className="border-r border-black"></td>
                <td className="border-r border-black p-1.5 text-center">{items.reduce((s, d) => s + d.qty, 0)}</td>
                <td className="border-r border-black"></td>
                <td className="border-r border-black"></td>
